...
// BenchmarkAggregateRelation compares identical nested-relation SQL and result
// types through ScanAll, typed Raw, and database/sql. The driver does not
// evaluate predicates or SQL aggregation.
func BenchmarkAggregateRelation(b *testing.B) {
	for _, count := range []int{0, 1, 100, 10000} {
		b.Run(fmt.Sprintf("rows_%d", count), func(b *testing.B) {
			q := Aggregate[aggregateRelationOrder]().Select(Field("ShopID"), CountAll().As("Count"), Sum("Amount").As("Total")).Where(Has("User", Equal("Active", true), Has("Orders", GreaterThan("Amount", int64(50))))).GroupBy("ShopID").Having(GreaterThan("Count", int64(0))).OrderBy(Asc("ShopID"))
